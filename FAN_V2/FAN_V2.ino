...
int currentSpeed = 0;
int lastSentSpeed = -1;  // Track last speed sent to Firestore
...
void loop() {
  ...
  if (pendingUpdate) {
    updateFirestoreState(currentSpeed, currentDeviceStatus);
  }

  // Check device status every second
  if (currentMillis - previousMillisStatusCheck >= intervalStatusCheck) {
    previousMillisStatusCheck = currentMillis; // Save the last time we checked the status
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if (currentMillis - previousMillisDataCheck >= intervalDataCheck) {
      ...
      if (isnan(humidity) || isnan(temperature)) {
        ...
        sendToFirestore(temperature, humidity); // Send data to Firestore
      }
    }
  } else {
    digitalWrite(ledPin, HIGH);
...
void updateFirestoreState(int speed, String status) {
  unsigned long currentTime = millis();
  
  // Only update if values have changed and enough time has passed
  if ((speed != lastSentSpeed || status != lastSentStatus) && 
    ...
    String jsonPayload = "{\"fields\": {\"setFanSpeed\": {\"integerValue\": " + String(speed) + 
...
void manualFanControl() {
  ...
  if (readingUp == LOW && !buttonUpPressed && 
      (currentTime - lastDebounceTimeUp) > debounceDelay) {
    if (currentSpeed < 4) {
      currentSpeed++;
  ...
  if (readingDown == LOW && !buttonDownPressed && 
      (currentTime - lastDebounceTimeDown) > debounceDelay) {
    if (currentSpeed > 0) {
      currentSpeed--;
      currentDeviceStatus = (currentSpeed == 0) ? "OFF" : "ON";
      stateChanged = true;
...
// Modified checkDeviceStatus function to respect manual controls
  ...
  if (currentTime - lastFirestoreCheck >= firestoreCheckInterval && !pendingUpdate) {
    ...
    if (httpResponseCode > 0) {
      ...
      if (!error) {
        ...
        int newFanSpeed = doc["fields"]["setFanSpeed"]["integerValue"].as<int>();
        
        if (newDeviceStatus != currentDeviceStatus || newFanSpeed != currentSpeed) {
          currentDeviceStatus = newDeviceStatus;
          currentSpeed = newFanSpeed;
          fanSpeedControl(currentSpeed);
          preferences.putInt("current_speed", currentSpeed);
...
void setupAP(void) {
  ...
  WiFi.softAP(("HomeTalk_Fan" + deviceName).c_str(), "");
